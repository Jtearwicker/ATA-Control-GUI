...
class ATAObservationGUI:
    ...
    def _build_layout(self):
        ...
        # Inside main_frame: left controls + right side (status + spectrum)
    ...
    def _build_spectrum_tab(self, parent):
        ...
        self.spec_figure = Figure(figsize=(5, 4), dpi=100)
    ...
    def start_data_stream(self):
        ...
        self.spec_address_var.set(address)

        # Start the external GNU Radio script locally
        try:
            ...
            self.log(f"Failed to start spectrum script: {e}")
            messagebox.showerror("Error", f"Could not start spectrum script:\n{e}")
            self.stream_process = None
            return

        # Prepare ZMQ receiving
        self.spectrum_stop_event.clear()
        self.spectrum_first_frame_received = False
        self.spectrum_running = True
        self.spectrum_status_label.configure(
            text="Starting data stream, please wait...",
            text_color="orange",
        )

        # Clear any old spectra
        with self.spectrum_queue.mutex:
            self.spectrum_queue.queue.clear()

        t = threading.Thread(
            target=self._spectrum_worker, args=(address,), daemon=True
        )
        self.spectrum_thread = t
        t.start()

    def _spectrum_worker(self, address: str):
        """Background thread: subscribe to ZMQ and push spectra into the queue."""
        ctx = None
        sock = None
        try:
            ctx = zmq.Context()
            ...
            while not self.spectrum_stop_event.is_set():
                ...
                psd = np.frombuffer(msg, dtype=np.float32)
                if psd.size == 0:
                    continue

                # First frame → update label to "active"
                if not self.spectrum_first_frame_received:
                    self.spectrum_first_frame_received = True
                    self.root.after(
                        0,
                        lambda: self.spectrum_status_label.configure(
                ...
                try:
                    self.spectrum_queue.put(psd, block=False)
                except queue.Full:
            ...
            self.root.after(0, lambda: self.log(f"Spectrum thread error: {e}"))
        finally:
            if sock is not None:
                try:
                    sock.close(0)
    ...
    def stop_data_stream(self):
        """Stop ZMQ streaming and the external GNU Radio script."""
        if not self.spectrum_running and self.stream_process is None:
            self.log("Data stream is not running.")
            self.spectrum_status_label.configure(
                text="Data stream not running",
                text_color="red",
            )
